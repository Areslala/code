...
class VOCDetection(data.Dataset):
    ...
    def __init__(self,
        ...
        for (year, name) in image_sets:
            self._year = year
            rootpath = os.path.join(self.root, "barcode")
            for line in open(
    ...
    def pull_item(self, index):
        ...
        if self.name != 'test':
            if self.transform is not None:
                img, target = self.transform(img, target)  #transform后，所有图片的size变为320*320，并且bbox坐标归一化了
#                 image_shape = img.shape
#                 print("h:",image_shape[2])
#                 print("w:",image_shape[1])
#                 print(target)
        else:
            if self.transform is not None:
